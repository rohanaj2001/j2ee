...
String name,usn,address,category; int age;
float cgpa;
public Student(String name,String usn,String address,String cat,int age, float cgpa) { this.name=name;
this.usn=usn;
this.address=address;
this.category=cat;
this.age=age; this.cgpa=cgpa; }
public String toString() {
String stud= name + " "+usn+" residing in "+address+" belonging to category "+category+" of age "+age; stud+= "has cgpa "+cgpa; return stud; }
}
import java.awt.*;
...
LinkedList stud_list = new LinkedList();
...
f1.add(jl4);f1.add(cat); f1.add(jl5);f1.add(sgpa1);
f1.add(jl6);f1.add(sgpa2);
f1.add(jl7);f1.add(sgpa3);
1.add(jl8 );f1.add(sgpa4);
f1.add(jl9);f1.add(sgpa5);
f1.add(jl0);f1.add(sgpa6);
f1.add(jl11);f1.add(sgpa7);
f1.add(jl 12);f1.add(sgpa8);
f1.add(jcgpa);f1.add(cgpa);
f1.add(submit);f1.add(done);
f1.add(display); f2.add(stud_list_display);
f1.setSize(900,800);
...
try { int v1=Integer.parseInt(age.getText()); if(v135) {
String age1=JOptionPane.showInputDialog(null,"Enter valid Age");
age.setText(age1); }
} catch(NumberFormatException e) {
JOptionPane.showMessageDialog(f1, "Invalid entry"); age.requestFocus(); } checkSGPA_valid(1,sgpa1);
...
{ Student s1 = new Student(name.getText(),usn.getText(),address.getText(),String.valueOf(cat.getSelectedItem()), Integer.parseInt(age.getText()),Float.parseFloat(cgpa.getText())); stud_list.add(s1); }
else {
f1.setVisible(false);
f2.setVisible(true);
f2.setSize(500,500);
...
try { if(Float.parseFloat(sgpa.getText())>10) {
String v1 = JOptionPane.showInputDialog(null,"Enter an SGPA less than or equal to 10 for sem "+sem); sgpa.setText(v1); } } catch(NumberFormatException e) {
String v2=JOptionPane.showInputDialog(null, "Please enter SGPA for semester "+sem); sgpa.setText(v2); //sgpa.requestFocus(); }
}
